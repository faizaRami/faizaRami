...
if(p!=null){
	if(mymodels.Role.ADMIN.equals(p.getRole())) {
		response.sendRedirect("http://localhost:8282/FaizaRami/admin");
	}else if(mymodels.Role.ETUDIANT.equals(p.getRole())) {
...
<head>
  ...
  <title>Ma page D'authentification</title>
    ...
    <div class="container">
      <form class="form-login" action="Authentification" method="post">
        ...
        <div class="login-wrap">
          <input type="text" class="form-control" name="email" placeholder="Votre adresse email" autofocus>
          <br>
          <input type="password" class="form-control" name="mdp" placeholder="Votre mot de passe">
          <br>
          <input class="btn btn-theme btn-block" type="submit" value="S'authentifier">
          <hr>
          
          <div class="registration">
           Vous n'avez pas un compte?<br/>
            <a class="" href="http://localhost:8282/FaizaRami/inscrire.jsp">
              Créer un compte
              </a>
          </div>
        </div>
